...
{
    constexpr bool is_valid_gfunction_flag()
    {
        return true;
    }
...
#define CHECK_GFUNCTION_FLAG(flag) \
    static_assert(GDGenInternal::is_valid_gfunction_flag(#flag), "Invalid GFUNCTION flag: " #flag);

#define VALIDATE_EACH_GFUNCTION_FLAG(...) FOR_EACH(CHECK_GFUNCTION_FLAG, SEMICOLON_END, __VA_ARGS__)

#define GFUNCTION(...) \
    VALIDATE_EACH_GFUNCTION_FLAG(__VA_ARGS__)